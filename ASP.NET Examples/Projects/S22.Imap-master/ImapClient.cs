...
namespace S22.Imap {
	...
	public class ImapClient : IImapClient
	{
		...
		public MailMessage GetMessage(uint uid, bool seen = true, string mailbox = null) {
			return GetMessage(uid, FetchOptions.Normal, seen, mailbox).msg;
		}
		...
		public MailMessageX GetMessage(uint uid, FetchOptions options,
			...
			switch (options) {
				...
				case FetchOptions.NoAttachments:
					m.msg = GetMessage(uid, p => { return p.Disposition.Type !=
						ContentDispositionType.Attachment; }, seen, mailbox);
                    return m;
				case FetchOptions.TextOnly:
					m.msg = GetMessage(uid, p => { return p.Type == ContentType.Text; },
						seen, mailbox);
                    return m;
				default:
					m.msg = MessageBuilder.FromMIME822(GetMessageData(uid, seen, mailbox));
                    return m;
			}
		}
		...
		public MailMessage GetMessage(uint uid, ExaminePartDelegate callback,
			...
			lock (sequenceLock) {
				...
				MailMessage message = MessageBuilder.FromHeader(header).msg;
		...
		public MailMessage[] GetMessages(uint[] uids, FetchOptions options,
			bool seen = true, string mailbox = null) {
				List<MailMessage> list = new List<MailMessage>();
				foreach (uint uid in uids)
					list.Add(GetMessage(uid, options, seen, mailbox).msg);
				return list.ToArray();
		}
        ...
        {
            public int uid;
            public MailMessageX msg;

            public x()
            {
                msg = new MailMessageX();
            }
        }

        public List<x> GetMailHeaders(uint[] uids, bool seen = true, string mailbox = null)
        {
            if (!Authed)
                throw new NotAuthenticatedException();
            lock (sequenceLock)
            {
                ...
                SelectMailbox(mailbox);
                string tag = GetTag();
                List<x> msgs = new List<x>();
                string response = SendCommandGetResponse(tag + "FETCH 1:* (BODY" +
                    (seen ? null : ".PEEK") + "[HEADER])", false);
                int count = 0;
                while (response.StartsWith("*"))
                {
