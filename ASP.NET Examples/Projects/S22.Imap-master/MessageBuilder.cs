	...
	internal static class MessageBuilder {
		...
		internal static MailMessageX FromHeader(string text) {
			...
			if (ma.Success) {
				...
				m.msg.SubjectEncoding = Util.GetEncoding(ma.Groups[1].Value);
				try {
					m.msg.Subject = Util.DecodeWords(header["Subject"]).
						Replace("\n", "").Replace("\r", "");
				} catch {
					// If, for any reason, decoding fails, set the subject to the
					// original, unaltered string.
                    m.msg.Subject = header["Subject"];
				}
			} else {
                m.msg.SubjectEncoding = Encoding.ASCII;
                m.msg.Subject = header["Subject"];
			}
            m.msg.Priority = ParsePriority(header["Priority"]);
		...
		internal static MailMessage FromMIME822(string text) {
			...
			MailMessage m = FromHeader(header.ToString()).msg;
		...
		private static void SetAddressFields(MailMessageX m, NameValueCollection header) {
			MailAddress[] addr;
			if (header["To"] != null) {
			...
			if (header["Reply-to"] != null) {
				...
				foreach (MailAddress a in addr)
                    m.msg.ReplyToList.Add(a);
			}
            if (header["Received"] != null)
            {
                Regex r = new Regex(".+[,][ \t]+(?<Time>[0-9]+[ ][A-z]+[ ][0-9]+[ ][0-9]+[:][0-9]+[:][0-9]+).+");
                m.Received = Convert.ToDateTime(r.Match(header["Received"]).Groups["Time"].Value.ToString());
            }
		}
