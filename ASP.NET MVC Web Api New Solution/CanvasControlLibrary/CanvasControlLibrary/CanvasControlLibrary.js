...
function animatedDraw() {
    for (var i = 0; i < windowWithAnimationCount.length; i++) {
        ...
        if (wprops) {
            invalidateRect(windowWithAnimationCount[i].CanvasID, -1, wprops.X, wprops.Y, wprops.Width, wprops.Height);
        }
    }
}
...
function pointEvent(eventArray, canvasId, e, parentwindowid, suppressPreventDefault) {
                ...
                for (var u = 0; u < eventArray.length; u++) {
                    if (eventArray[u][0] == windows[i].WindowCount) {
                        ...
                        if (!donotredaw && windows[i]) {
                            invalidateRect(canvasId, -1, windows[i].X, windows[i].Y, windows[i].Width, windows[i].Height);
                            if (windows[i].ControlType == 'ScrollBar') {
                                ...
                                if (parentofscrollbarwindowprops) {
                                    invalidateRect(canvasId, -1, parentofscrollbarwindowprops.X, parentofscrollbarwindowprops.Y,
    ...
    if (dodraw == 1 && !donotredaw) {
        for (var i = 0; i < dodrawforwindowids.length; i++) {
            ...
            if (wprops) {
                invalidateRect(canvasId, -1, wprops.X, wprops.Y, wprops.Width, wprops.Height);
            }
        }
    }
    donotredaw = null;
...
function canvasOnClick(canvasId, e) {
    doingClickEvent = 1;
    pointEvent(clickFunctions, canvasId, e, -1);
    doingClickEvent = 0;
}

function canvasOnDblClick(canvasId, e) {
    pointEvent(doubleClickFunctions, canvasId, e, -1);
}

function canvasOnDrag(canvasId, e) {
    pointEvents(dragFunctions, canvasId, e, -1);
}

function canvasOnDragEnd(canvasId, e) {
    pointEvent(dragEndFunctions, canvasId, e, -1);
}

function canvasOnDragEnter(canvasId, e) {
    pointEvent(dragEnterFunctions, canvasId, e, -1);
}

function canvasOnDragLeave(canvasId, e) {
    pointEvent(dragLeaveFunctions, canvasId, e, -1);
}

function canvasOnDragOver(canvasId, e) {
    pointEvent(dragOverFunctions, canvasId, e, -1);
}

function canvasOnDragStart(canvasId, e) {
    pointEvent(dragStartFunctions, canvasId, e, -1);
}

function canvasOnDrop(canvasId, e) {
    pointEvent(dropFunctions, canvasId, e, -1);
}

function canvasOnMouseDown(canvasId, e) {
    doingMouseDown = 1;
    pointEvent(mouseDownFunctions, canvasId, e, -1);
    doingMouseDown = 0;
}

function canvasOnMouseMove(canvasId, e) {
    pointEvent(mouseMoveFunctions, canvasId, e, -1);
}

function canvasOnMouseOut(canvasId, e) {
    pointEvent(mouseOutFunctions, canvasId, e, -1);
}
...
function canvasOnMouseUp(canvasId, e) {
    doingMouseUp = 1;
    pointEvent(mouseUpFunctions, canvasId, e, -1);
    doingMouseUp = 0;
}

function canvasOnMouseWheel(canvasId, e) {
    pointEvent(mouseWheelFunctions, canvasId, e, -1);
}

function canvasOnScroll(canvasId, e) {
    pointEvent(scrollFunctions, canvasId, e, -1);
}
...
function registerCanvasElementId(canvasId) {
        ...
        for (var i = 0; i < keyPressFunctions.length; i++) {
            for (var j = 0; j < windowIdWithFocus.length; j++) {
                if (windowIdWithFocus[j][0] == keyPressFunctions[i].CanvasID && windowIdWithFocus[j][1] == keyPressFunctions[i].WindowID) {
                    ...
                    if (wprops) {
                        invalidateRect(keyPressFunctions[i].CanvasID, -1, wprops.X, wprops.Y, wprops.Width, wprops.Height);
                    }
                    if (e.preventDefault)
                        e.preventDefault();
                    e.returnValue = false;
        ...
        for (var i = 0; i < keyDownFunctions.length; i++) {
            for (var j = 0; j < windowIdWithFocus.length; j++) {
                if (e.keyCode == 9) {
                        ...
                        if (wprops.ControlType != 'WordProcessor') {
                            ...
                            for (var k = 0; k < windows.length; k++) {
                                if (windows[k].WindowCount != wprops.WindowCount && wprops.TabStopIndex + 1 == windows[k].TabStopIndex) {
                                    ...
                                    invalidateRect(windows[k].CanvasID, -1, 0, 0, can.width, can.height);
                                    if (e.preventDefault)
                                        e.preventDefault();
                                    e.returnValue = false;
                                    return false;
                    ...
                    } else if (wprops.MultiWindowControlsMainWindowId != -1 && wprops.MultiWindowControlsMainWindowId >= 0) {
                ...
                if (windowIdWithFocus[j][0] == keyDownFunctions[i].CanvasID && windowIdWithFocus[j][1] == keyDownFunctions[i].WindowID) {
                    ...
                    if (wprops) {
                        invalidateRect(keyDownFunctions[i].CanvasID, -1, wprops.X, wprops.Y, wprops.Width, wprops.Height);
                    }
                    if (e.preventDefault)
                        e.preventDefault();
                    e.returnValue = false;
    ...
    if (navigator.userAgent.toLowerCase().indexOf('android') > -1 || navigator.userAgent.toLowerCase().indexOf('ipad') > -1 || navigator.userAgent.toLowerCase().indexOf('iphone') > -1 || navigator.userAgent.toLowerCase().indexOf('ipod') > -1) {
        ...
        canvas.addEventListener("touchend", function (e) {
            e.pageX = e.touches[0].pageX;
            e.pageY = e.touches[0].pageY;
            pointEvent(mouseUpFunctions, canvasId, e, -1, 1);
        }, false);
    } else {
...
CCLLabel.prototype = {
    ...
    Alignment: null, ClickFunction: null, BackGroundColor: null, Tag: null, TabStopIndex: -1,
...
function createLabel(canvasid, controlNameId, x, y, width, height, text, textColor, textHeight, textFontString, drawFunction, depth,
    ...
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'Label', controlNameId, -1, tabstopindex);
    ...
    labelPropsArray.push({
        ...
        BackGroundColor: backgroundColor, Tag: tag
    });
...
CCLButton.prototype = {
    ...
    ControlNameID: null, Depth: null, ClickFunction: null, DrawFunction: null, TabStopIndex: -1,
...
function createButton(canvasid, controlNameId, x, y, width, height, text, textColor, textHeight, textFontString, edgeRadius, depth, theme, hasgloss, clickFunction,
    ...
    newbrowserwindowhasstatusbar, newbrowserwindowhasmenubar, newbrowserwindowcopyhistory, tabstopindex) {
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'Button', controlNameId, -1, tabstopindex);
...
CCLScrollbar.prototype = {
    ...
    ClickFunction: null, SelectedTag: null, TabStopIndex: -1,
    ...
    Initialize: function () {
        return createScrollBar(this.CanvasID, this.ControlNameID, this.X, this.Y, this.Len, this.Depth, this.MaxItems,
            ...
            this.CustomIncrementFunction, this.SelectedTag, this.CustomMouseMoveFunction, this.TabStopIndex)
    }
}
...
function createScrollBar(canvasid, controlNameId, x, y, len, depth, maxitems, alignment, ownedbywindowid, drawFunction, clickFunction, tag,
    customIncrementFunction, selectedTag, customMouseMoveFunction, multiWindowControlsMainWindowId, tabstopindex) {
    var windowid;
    if (alignment == 1) {
        windowid = createWindow(canvasid, x, y, 15, len, depth, -1, 'ScrollBar', controlNameId, multiWindowControlsMainWindowId, tabstopindex);
    } else {
        windowid = createWindow(canvasid, x, y, len, 15, depth, -1, 'ScrollBar', controlNameId, multiWindowControlsMainWindowId, tabstopindex);
    }
...
CCLGrid.prototype = {
    ...
    DrawRowDataCellFunction: null, RowDataTextHeight: null, Depth: null, TabStopIndex: -1,
...
function createGrid(canvasid, controlNameId, x, y, width, height, depth, rowData, headerData, rowDataTextColor, rowDataTextHeight, rowDataTextFontString,
    ...
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'Grid', controlNameId, -1, tabstopindex);
    ...
    gridPropsArray.push({
        ...
        HeaderDataTextHeight: headerDataTextHeight, HeaderDataTextFontString: headerDataTextFontString, DataRowHeight: dataRowHeight,
...
function clickGrid(canvasid, windowid, e) {
    ...
    if (sortingChanged == 1) {
        ...
        invalidateRect(gridProps.CanvasID, -1, gridProps.X, gridProps.Y, gridProps.Width, gridProps.Height);
    }
}
...
CCLCombobox.prototype = {
    ...
    DrawTextAreaFunction: null, Depth: null, TabStopIndex: -1,
...
function createComboBox(canvasid, controlNameId, x, y, width, height, depth, data, drawTextAreaFunction, drawButtonFunction, drawListAreaFunction, buttonClickFunction,
    ...
    tabstopindex) {
    var textareawindowid = createWindow(canvasid, x, y, width - height, height, depth, -1, 'ComboBoxTextArea', controlNameId + 'ComboBoxTextArea', -1, tabstopindex);
    var buttonwindowid = createWindow(canvasid, x + width - height, y, height, height, depth, -1, 'ComboBoxButton', controlNameId + 'ComboBoxButton', textareawindowid, tabstopindex);
    var dropdownlistareawindowid = createWindow(canvasid, x, y + height, width - 15, 100, depth, -1, 'ComboBoxListArea', controlNameId + 'ComboBoxListArea', textareawindowid, tabstopindex);
        ...
        registerKeyDownFunction(canvasid, function (canvasid, windowid, e) {
            if (e.keyCode == 40) {
                ...
                if (checkIfHiddenWindow(canvasid, comboboxProps.ListAreaWindowID) == 1) {
                    ...
                    invalidateRect(canvasid, -1, comboboxProps.X, comboboxProps.Y, comboboxProps.Width + 1, comboboxProps.Height + 101);
                }
    ...
    for (var i = vscrollBarProps.SelectedID; i < comboboxProps.Data.length && ((comboboxProps.ListAreaTextHeight + 6) * (i - vscrollBarProps.SelectedID + 1)) < 100; i++) {
        if (x > comboboxProps.X && y > comboboxProps.Y + comboboxProps.Height + ((comboboxProps.ListAreaTextHeight + 6) * (i - vscrollBarProps.SelectedID)) &&
            ...
            if (comboboxProps.SelectedID != i) {
                ...
                setHiddenWindowStatus(canvasid, comboboxProps.ListAreaWindowID, 1);
                var canvas = getCanvas(canvasid);
                invalidateRect(canvasid, -1, 0, 0, canvas.width, canvas.height);
...
CCLCheckbox.prototype = {
    CanvasID: null, X: null, Y: null, Status: null, Tag: null, ControlNameID: null, Depth: null, TabStopIndex: -1,
...
function createCheckbox(canvasid, controlNameId, x, y, depth, status, tag, tabstopindex) {
    var windowid = createWindow(canvasid, x, y, 15, 15, depth, -1, 'CheckBox', controlNameId, -1, tabstopindex);
...
CCLRadioButtonGroup.prototype = {
    ...
    LabelTextHeight: null, Tag: null, ControlNameID: null, Depth: null, TabStopIndex: -1,
...
function createRadioButtonGroup(canvasid, controlNameId, x, y, alignment, depth, groupname, labels, selectedid, labelTextColor,
    ...
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'RadioButtonGroup', controlNameId, -1, tabstopindex);
...
CCLImage.prototype = {
    ...
    ControlNameID: null, Depth: null, TabStopIndex: -1,
...
function createImage(canvasid, controlNameId, x, y, width, height, depth, imgurl, clickFunction, tile, tag,
    ...
    newbrowserwindowhastoolbar, newbrowserwindowhaslocationorurloraddressbox, newbrowserwindowhasdirectoriesorextrabuttons,
    newbrowserwindowhasstatusbar, newbrowserwindowhasmenubar, newbrowserwindowcopyhistory, tabstopindex) {
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'Image', controlNameId, -1, tabstopindex);
    var image = new Image();
    image.onload = function () {
        invalidateRect(canvasid, -1, x, y, width, height);
    };
...
function treeviewVSCustomIncrementFunction(currNode, scrollbarProps, incordec) {
    ...
    for (var i = 0; i < treeViewPropsArray.length; i++) {
        if (getTreeviewNodes(treeViewPropsArray[i]) == currNode.TreeviewNodeInstancesRootNodes) {
            invalidateRect(treeViewPropsArray[i].CanvasID, -1, treeViewPropsArray[i].X, treeViewPropsArray[i].Y, treeViewPropsArray[i].Width, treeViewPropsArray[i].Height);
...
CCLTreeview.prototype = {
    ...
    ControlNameID: null, Depth: null, Nodes: null, SelectedNode: null, TabStopIndex: -1,
...
function createTreeView(canvasid, controlNameId, x, y, width, height, depth, nodes,
    ...
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'TreeView', controlNameId, -1, tabstopindex);
...
function clickTreeView(canvasid, windowid, e) {
    ...
    for (var i = 0; i < clickButtonExtents.length; i++) {
        if (clickButtonExtents[i].Node && x > clickButtonExtents[i].X &&
            ...
            invalidateRect(canvasid, -1, treeViewProps.X, treeViewProps.Y, treeViewProps.Width, treeViewProps.Height);
            return;
        }
    }
...
function createCalendar(canvasid, controlNameId, x, y, width, height, depth, visibleMonth, visibileYear, selectedDay, dayCellWidth, dayCellHeight, headerHeight,
    ...
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'Calender', controlNameId, -1, tabstopindex);
...
CCLProgressBar.prototype = {
    ...
    MinValue: null, CurrentValue: null, Tag: null, ControlNameID: null, Depth: null, TabStopIndex: -1,
...
function createProgressBar(canvasid, controlNameId, x, y, width, height, depth, color, maxvalue, minvalue, currentvalue, tag, tabstopindex) {
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'ProgressBar', controlNameId, -1, tabstopindex);
...
function setProgressBarCurrentValue(canvasid, windowid, value) {
    ...
    invalidateRect(canvasid, -1, progressBarProps.X, progressBarProps.Y, progressBarProps.Width, progressBarProps.Height);
}
...
CCLSlider.prototype = {
    ...
    ControlNameID: null, Depth: null, TabStopIndex: -1, Value: null,
...
function createSlider(canvasid, controlNameId, x, y, width, height, depth, handlewidth, maxvalue, minvalue, value, tag, tabstopindex) {
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'Slider', controlNameId, -1, tabstopindex);
...
CCLDatePicker.prototype = {
    ...
    ControlNameID: null, Depth: null, TabStopIndex: -1, VisibleMonth: null, VisibleYear: null,
    SelectedDay: null, DayCellWidth: null, DayCellHeight: null, HeaderHeight: null,
    TextHeaderColor: null, TextHeaderHeight: null, TextHeaderFontString: null,
    DayDateActiveColor: null, DayDateActiveTextHeight: null,
    DayDateActiveTextFontString: null, DayDateInactiveTextColor: null,
...
function createDatePicker(canvasid, controlNameId, x, y, width, height, depth, visibleMonth, visibileYear, selectedDay, dayCellWidth, dayCellHeight, headerHeight,
    ...
    var textboxAreaWindowID = createWindow(canvasid, x, y, width - height, height, depth, -1, 'DatePickerTextArea', controlNameId + 'DatePickerTextArea', -1);
    var buttonWindowID = createWindow(canvasid, x + width - height, y, height, height, depth, -1, 'DatePickerButton', controlNameId + 'DatePickerButton', -1);
    ...
    registerClickFunction(buttonWindowID, function (canvasid2, windowid2) {
        ...
        invalidateRect(canvasid2, -1, dropdowncalprops.X, dropdowncalprops.Y, dropdowncalprops.Width, dropdowncalprops.Height);
    }, canvasid);
...
function createPanel(canvasid, controlNameId, x, y, width, height, depth, hasBorder, borderColor, hasBackgroundGradient, backgroundStartColor, backgroundEndColor,
    ...
    var windowid = createWindow(canvasid, x, y, (iscollapsable == 1 ? (isexpanded == 1 ? width : collapsedWidth) : width),
        (iscollapsable == 1 ? (isexpanded == 1 ? height : headerheight) : height), depth, -1, 'Panel', controlNameId, -1, tabstopindex);
    ...
    if (iscollapsable == 1) {
        registerClickFunction(windowid, function (canvasid3, windowid3, e) {
            ...
            if (windowProps) {
                ...
                invalidateRect(canvasid3, -1, panelProps.X, panelProps.Y, panelProps.OriginalWidth, panelProps.OriginalHeight);
            }
        }, canvasid);
    } else {
...
CCLBarGraph.prototype = {
    ...
    HasLegend: null, MarginRight: null, Tag: null, ControlNameID: null, Depth: null, TabStopIndex: -1,
...
function createBarGraph(canvasid, controlNameId, x, y, width, height, depth, data, maxvalue, nummarksy, title, titletextcolor,
    ...
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'BarGraph', controlNameId, -1, tabstopindex);
...
CCLPieChart.prototype = {
    ...
    SliceClickFunction: null, Tag: null, ControlNameID: null, Depth: null, TabStopIndex: -1,
...
function createPieChart(canvasid, controlNameId, x, y, width, height, depth, data, title, titletextcolor, titletextheight, titletextfontstring,
    ...
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'PieChart', controlNameId, -1, tabstopindex);
    var totalvalue = 0;
...
function createLineGraph(canvasid, controlNameId, x, y, width, height, depth, data, xmaxvalue, nummarksx, ymaxvalue, nummarksy, title,
    ...
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'LineGraph', controlNameId, -1, tabstopindex);
    var hmax = 0;
...
CCLGauge.prototype = {
    ...
    GaugeLabelTextFontString: null, Tag: null, ControlNameID: null, Depth: null, TabStopIndex: -1,
...
function createGauge(canvasid, controlNameId, x, y, width, height, depth, data, title, titletextcolor, titletextheight, titletextfontstring, gaugeradius,
    ...
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'Gauge', controlNameId, -1, tabstopindex);
...
CCLRadarGraph.prototype = {
    ...
    Tag: null, ControlNameID: null, Depth: null, TabStopIndex: -1,
...
function createRadarGraph(canvasid, controlNameId, x, y, width, height, depth, data, maxvalue, colorstr, nummarks, title, titletextcolor, titletextheight,
    ...
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'RadarGraph', controlNameId, -1, tabstopindex);
...
CCLLineAreaGraph.prototype = {
    ...
    MarginLeft: null, IsLabledOnXAxis: null, Tag: null, ControlNameID: null, Depth: null, TabStopIndex: -1,
...
function createLineAreaGraph(canvasid, controlNameId, x, y, width, height, depth, data, xmaxvalue, ymaxvalue, nummarksx, nummarksy, title,
    ...
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'LineAreaGraph', controlNameId, -1, tabstopindex);
...
function createCandlesticksGraph(canvasid, controlNameId, x, y, width, height, depth, data, xmarkslabeldata, xmarkswidth, ymaxvalue, nummarksy, title,
    ...
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'CandlesticksGraph', controlNameId, -1, tabstopindex);
...
function createDoughnutChart(canvasid, controlNameId, x, y, width, height, depth, data, title, titlecolor, titletextheight, titlefontstring, innerradius, marginsides,
    ...
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'DoughnutChart', controlNameId, -1, tabstopindex);
    var totalvalue = 0;
    for (var i = 0; i < data.length; i++) {
        totalvalue += data[i][1];
    }
...
CCLBarsMixedWithLabledLineGraph.prototype = {
    ...
    YMaxValue: null, Tag: null, ControlNameID: null, Depth: null, TabStopIndex: -1, LinesData: null,
...
function createBarsMixedWithLabledLineGraph(canvasid, controlNameId, x, y, width, height, depth, data, maxvalue, nummarksy, title, titletextcolor,
    ...
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'BarsMixedWithLabeledLineGraph', controlNameId, -1, tabstopindex);
...
CCLStackedBarGraph.prototype = {
    ...
    AxisLabelsColor: null, AxisLabelsHeight: null, AxisLabelsFontString: null,
    BarClickFunction: null, MarginLeft: null, Tag: null, ControlNameID: null, Depth: null, TabStopIndex: -1,
...
function createStackedBarGraph(canvasid, controlNameId, x, y, width, height, depth, data, maxvalue, nummarksy, title, titlecolor, titleheight,
    ...
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'StackedBarGraph', controlNameId, -1, tabstopindex);
...
CCLTabControl.prototype = {
    ...
    SelectedTabBorderLineWidth: null, Tag: null, ControlNameID: null, Depth: null, TabStopIndex: -1,
...
function createTabControl(canvasid, controlNameId, x, y, width, height, depth, tablabels, tablabelcolor, tablabelheight, tablabelfontstring,
    ...
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'Tab', controlNameId, -1, tabstopindex);
...
function createImageMapControl(canvasid, controlNameId, x, y, width, height, depth, imgurl, pinxys, pinClickFunction, hasZoom,
    ...
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'ImageMap', controlNameId, -1, tabstopindex);
    var image = new Image();
    image.src = imgurl;
    image.onload = function () {
        invalidateRect(canvasid, -1, x, y, width, height);
...
CCLSubMenu.prototype = {
    ...
    DropDownColorStart: null, DropDownColorEnd: null, Tag: null, ControlNameID: null, Depth: null, TabStopIndex: -1,
...
function createSubMenu(canvasid, controlNameId, parentWindowId, depth, data, xoffset, yoffset, parentIndexInParentMenu, dropdowncolorstart,
    ...
    var windowid = createWindow(canvasid, xoffset, yoffset, greatestLength, greatestHeight, depth, -1, 'SubMenu', controlNameId, -1, tabstopindex);
...
function createMenuBarControl(canvasid, controlNameId, x, y, width, height, depth, data, barcolorstart, barcolormiddle, barcolorend,
    ...
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'MenuBar', controlNameId, -1, tabstopindex);
    var ctx = getCtx(canvasid);
    var widthOffset = 0;
    var childMenuWindowIds = new Array();
    for (var i = 0; i < data.length; i++) {
...
function textBoxTouchKeyPress(canvasid, windowid, keyboardChar) {
    ...
    switch (keyboardChar.toLowerCase()) {
        case 'left':
            ...
            invalidateRect(canvasid, -1, textBoxProps.X, textBoxProps.Y, textBoxProps.Width, textBoxProps.Height);
            return;
        case 'right':
            //right arrow	 39
...
CCLTextbox.prototype = {
    ...
    WaterMarkTextHeight: null, TabStopIndex: -1,
...
function createTextBox(canvasid, controlNameId, x, y, width, height, depth, waterMarkText, waterMarkTextColor, waterMarkTextHeight, waterMarkTextFontString,
    ...
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'TextBox', controlNameId, -1, tabstopindex);
    var dropdownwindowid, vscrollbarwindowid;
    if (hasAutoComplete == 1) {
        dropdownwindowid = createWindow(canvasid, x, y + height, width, 100, depth, -1, 'TextBoxDropDown', controlNameId + 'TextBoxDropDown', -1);
...
CCLImageFader.prototype = {
    ...
    OverlayImages: null, ControlNameID: null, Depth: null, TabStopIndex: -1,
...
function createImageFader(canvasid, controlNameId, x, y, width, height, depth, imageURLs, fadeStartValue, fadeEndValue, fadeStepValue, holdForTicks,
    ...
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'ImageFader', -1, tabstopindex);
...
function createImageSlider(canvasid, controlNameId, x, y, width, height, depth, imageURLs, direction, stepIncrement, holdForTicks, clickFunction, tabstopindex) {
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'ImageSlider', controlNameId, -1, tabstopindex);
...
function createMultiLineLabel(canvasid, controlNameId, x, y, width, depth, hasMarkup, text, textColor, textHeight, textFontString,
    ...
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'MultiLineLabel', controlNameId, -1, tabstopindex);
...
CCLWordProcessor.prototype = {
    ...
    AllowedCharsRegEx: null, CustomKeyboardWindowID: null, ControlNameID: null, Depth: null, TabStopIndex: -1,
    ...
    Initialize: function () {
        return createWordProcessor(this.CanvasID, this.ControlNameID, this.X, this.Y, this.Width, this.Height,
            ...
            this.CustomKeyboardWindowID, this.TabStopIndex);
    }
}
...
function createWordProcessor(canvasid, controlNameId, x, y, width, height, depth, hasMarkup, text, textColor, textHeight, textFontString, lineSpacingInPixels, wordSensitive,
    ...
    allowedCharsRegEx, caretColor, customKeyboardWindowID, tabstopindex) {
    var windowid;
    if (hasMarkup == 1) {
        windowid = createWindow(canvasid, x, y + 20, width - 15, height - 20, depth, -1, 'WordProcessor', controlNameId, -1, tabstopindex);
    } else {
        windowid = createWindow(canvasid, x, y, width - 15, height, depth, -1, 'WordProcessor', controlNameId, -1, tabstopindex);
    }
...
function createVirtualKeyboard(canvasid, controlNameId, x, y, width, height, depth, keys, keypressFunc, gapbetweenbuttons,
    ...
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'VirtualKeyboard', controlNameId, -1, tabstopindex);
...
CCLSplitter.prototype = {
    CanvasID: null, X: null, Y: null, Width: null, Height: null, LineColor: null, ControlNameID: null, Depth: null, TabStopIndex: -1,
...
function createSplitter(canvasid, controlNameId, x, y, width, height, depth, linecolor, tabstopindex) {
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'Splitter', controlNameId, -1, tabstopindex);
    ...
    for (var i = 0; i < windows.length; i++) {
        if (windows[i].ParentWindowID == -1 && ((height > width && windows[i].X < x + width + 2 && windows[i].X + windows[i].Width > x - 2) ||
            ...
            registerMouseMoveFunction(windows[i].WindowCount, function (canvasid2, windowid2, e) {
                ...
                if (splitterProps.MouseDown == 1) {
                    ...
                    if (splitterProps.Height > splitterProps.Width) {
                        ...
                        if (diffX != 0) {
                            for (var i = 0; i < windows.length; i++) {
                                if (windows[i].WindowCount != windowid && windows[i].ParentWindowID == -1 &&
                            ...
                            windowProps.X = x;
                            splitterProps.X = x;
                            invalidateRect(canvasid, -1, irx, iry, maxirw - irx, maxirh - iry);
                        }
...
var boundaryFillableMapImages = new Array();
...
function getBoundaryFillableMapImage(canvasid, windowid) {
    for (var i = 0; i < boundaryFillableMapImages.length; i++) {
        if (boundaryFillableMapImages[i].CanvasID == canvasid && boundaryFillableMapImages[i].WindowID == windowid) {
            return boundaryFillableMapImages[i].Image;
        }
    }
}
...
CCLBoundaryFillableMap.prototype = {
    ...
    ImgURL: null, Image: null, ImageWidth: null, ImageHeight: null, ControlNameID: null, Depth: null, TabStopIndex: -1,
...
function createBoundaryFillableMap(canvasid, controlNameId, x, y, width, height, depth, fillpoints, imgurl, imgwidth, imgheight, tabstopindex) {
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'BoundaryFillableMap', controlNameId, -1, tabstopindex);
    var image = new Image();
    boundaryFillableMapImages.push({
        CanvasID: canvasid, WindowID: windowid, Image: image
    });
    image.onload = function () {
        invalidateRect(canvasid, -1, x, y, width, height);
    };
    ...
    boundaryFillableMapPropsArray.push({
        CanvasID: canvasid, WindowID: windowid, X: x, Y: y, Width: width, Height: height, FillPoints: fillpoints, ImgURL: imgurl, ImageWidth: imgwidth,
    ...
    registerWindowDrawFunction(windowid, function (canvasid1, windowid1) {
        ...
        var ctx = canvas.getContext('2d');
        ctx.drawImage(getBoundaryFillableMapImage(canvasid1, windowid1), 0, 0);
...
CCLVotingControl.prototype = {
    ...
    ControlNameID: null, Depth: null, TabStopIndex: -1, IsCustomPattern: null,
...
function createVotingControl(canvasid, controlNameId, x, y, width, height, depth, numstars, maxvalueofallstars, starcolorred, starcolorgreen, starcolorblue,
    ...
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'VotingControl', controlNameId, -1, tabstopindex);
...
CCLRibbon.prototype = {
    ...
    TabStopIndex: -1, BgColorStart: null, BgColorEnd: null, ScrollableLists: null,
...
function createRibbon(canvasid, controlNameId, x, y, width, height, depth, controls, tabstopindex, bgcolorstart, bgcolorend, scrollablelists) {
    var windowid = createWindow(canvasid, x, y, width, height, depth, -1, 'Ribbon', controlNameId, -1, tabstopindex);
...
function invokeServerSideFunction(canvasid, width, height, functionName, params, callback) {
    var data = JSON.parse(JSON.stringify({
        Windows: windows, LabelPropsArray: labelPropsArray, ButtonPropsArray: buttonPropsArray,
        ScrollBarPropsArray: scrollBarPropsArray, GridPropsArray: gridPropsArray,
        ComboBoxPropsArray: comboboxPropsArray, CheckBoxPropsArray: checkboxPropsArray,
        RadioButtonGroupPropsArray: radiobuttonPropsArray, ImagePropsArray: imageControlPropsArray,
        TreeViewPropsArray: treeViewPropsArray, CalenderPropsArray: calenderPropsArray,
        ProgressBarPropsArray: progressBarPropsArray, SliderPropsArray: sliderPropsArray,
        DatePickerPropsArray: datePickerPropsArray, PanelPropsArray: panelPropsArray,
        BarGraphPropsArray: barGraphsPropsArray, PieChartPropsArray: pieChartsPropsArray,
        LineGraphPropsArray: lineGraphsPropsArray, GaugeChartPropsArray: gaugeChartPropsArray,
        RadarGraphPropsArray: radarGraphPropsArray, LineAreaGraphPropsArray: lineAreaGraphPropsArray,
        CandlesticksGraphPropsArray: candlesticksGraphPropsArray, DoughnutChartPropsArray: doughnutChartPropsArray,
        BarsMixedWithLabeledLineGraphPropsArray: barsMixedWithLabledLineGraphsPropsArray,
        StackedBarGraphPropsArray: stackedBarGraphPropsArray, TabPropsArray: tabPropsArray,
        ImageMapPropsArray: imageMapPropsArray, MenuBarPropsArray: menuBarPropsArray,
        SubMenuBarPropsArray: subMenuBarPropsArray, TextBoxPropsArray: textBoxPropsArray,
        ImageFaderPropsArray: imageFaderPropsArray, ImageSliderPropsArray: imageSliderPropsArray,
        MultiLineLabelPropsArray: multiLineLabelPropsArray, WordProcessorPropsArray: wordProcessorPropsArray,
        VirtualKeyboardPropsArray: virtualKeyboardPropsArray, SplitterPropsArray: splitterPropsArray,
        BoundaryFillableMapProps: boundaryFillableMapPropsArray, SimpleXMLViewerProps: simpleXMLViewerPropsArray,
        VotingProps: votingPropsArray, FunctionName: functionName, Params: params, ReturnParams: new Array()
    }));
    $.post('api/CCLs', data, function (data, status, xhr) {
        if (status == 'success' && data != null) {
            windows = data.Windows;
            labelPropsArray = data.LabelPropsArray;
            buttonPropsArray = data.ButtonPropsArray;
            scrollBarPropsArray = data.ScrollBarPropsArray;
            gridPropsArray = data.GridPropsArray;
            comboboxPropsArray = data.ComboBoxPropsArray;
            checkboxPropsArray = data.CheckBoxPropsArray;
            radiobuttonPropsArray = data.RadioButtonGroupPropsArray;
            imageControlPropsArray = data.ImagePropsArray;
            treeViewPropsArray = data.TreeViewPropsArray;
            calenderPropsArray = data.CalenderPropsArray;
            progressBarPropsArray = data.ProgressBarPropsArray;
            sliderPropsArray = data.SliderPropsArray;
            datePickerPropsArray = data.DatePickerPropsArray;
            panelPropsArray = data.PanelPropsArray;
            barGraphsPropsArray = data.BarGraphPropsArray;
            pieChartsPropsArray = data.PieChartPropsArray;
            lineGraphsPropsArray = data.LineGraphPropsArray;
            gaugeChartPropsArray = data.GaugeChartPropsArray;
            radarGraphPropsArray = data.RadarGraphPropsArray;
            lineAreaGraphPropsArray = data.LineAreaGraphPropsArray;
            candlesticksGraphPropsArray = data.CandlesticksGraphPropsArray;
            doughnutChartPropsArray = data.DoughnutChartPropsArray;
            barsMixedWithLabledLineGraphsPropsArray = data.BarsMixedWithLabeledLineGraphPropsArray;
            stackedBarGraphPropsArray = data.StackedBarGraphPropsArray;
            tabPropsArray = data.TabPropsArray;
            imageMapPropsArray = data.ImageMapPropsArray;
            menuBarPropsArray = data.MenuBarPropsArray;
            subMenuBarPropsArray = data.SubMenuBarPropsArray;
            textBoxPropsArray = data.TextBoxPropsArray;
            imageFaderPropsArray = data.ImageFaderPropsArray;
            imageSliderPropsArray = data.ImageSliderPropsArray;
            multiLineLabelPropsArray = data.MultiLineLabelPropsArray;
            wordProcessorPropsArray = data.WordProcessorPropsArray;
            virtualKeyboardPropsArray = data.VirtualKeyboardPropsArray;
            splitterPropsArray = data.SplitterPropsArray;
            boundaryFillableMapPropsArray = data.BoundaryFillableMapProps;
            simpleXMLViewerPropsArray = data.SimpleXMLViewerProps;
            votingPropsArray = data.VotingProps;
            callback(data.ReturnParams);
        }
    });
}
